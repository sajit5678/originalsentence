...
@Js
def PyJsHoisted_processSelection_(this, arguments, var=var):
    var = Scope({'this':this, 'arguments':arguments}, var)
    var.registers(['count', 'i', 're', 'sentences', 'str', 'doubleAb', 'last4', 'last3', 'paragraphArray', 'shortHand', 'endHTML', 'allSentences', 'endCSV', 'tripleAb', 'j', 'k', 'numSentence', 'current', 'last6'])
    var.put('str', var.get('document').callprop('getElementById', Js('inputBox')).get('value'))
    ...
    for PyJsTemp in var.get('paragraphArray'):
        ...
        var.put('endCSV', Js(' ,'), '+')
    var.get('document').callprop('getElementById', Js('result')).put('innerHTML', var.get('endHTML'))
